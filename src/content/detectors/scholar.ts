...
export async function detectScholar(): Promise<PaperMetadata | null> {
  if (!window.location.hostname.includes('scholar.google.com')) {
    return null;
  }

  logger.debug('Detecting paper on Google Scholar');
  ...
  // Google Scholar uses citation meta tags
  const title = getMeta('title') || getMeta('citation_title');
  ...
  // Extract authors from citation_author tags (there can be multiple)
  ...
  // Extract year from citation_publication_date
  ...
  // Extract abstract (Scholar may not always have this in meta tags)
