...
export async function detectScienceDirect(): Promise<PaperMetadata | null> {
  if (!window.location.hostname.includes('sciencedirect.com')) {
    return null;
  }

  logger.debug('Detecting paper on ScienceDirect');
  ...
  // ScienceDirect uses citation meta tags
  ...
  // Extract publisher
  const publisher = getMeta('citation_publisher') || getMeta('dc.publisher') || 'Elsevier';
  ...
  const abstractEl = document.querySelector('#abstracts .abstract') ||
                     document.querySelector('.abstract.author') ||
