...
export async function detectJstor(): Promise<PaperMetadata | null> {
  if (!window.location.hostname.includes('jstor.org')) {
    return null;
  }

  logger.debug('Detecting paper on JSTOR');
  ...
  // Extract JSTOR ID
  const jstorId = getMeta('citation_jstor_id');

  // Extract abstract from page
  const abstractEl = document.querySelector('.abstract p') ||
                     document.querySelector('[data-qa="abstract"] p');
  ...
  if (!bibtexRaw) {
    ...
    // Add JSTOR ID as note if available
    if (jstorId) {
      metadata.note = `JSTOR ID: ${jstorId}`;
    }
  ...
  logger.debug('Paper detected:', { title, authors, year, doi, jstorId });
