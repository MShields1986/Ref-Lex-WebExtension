...
export async function detectIeee(): Promise<PaperMetadata | null> {
  if (!window.location.hostname.includes('ieeexplore.ieee.org')) {
    return null;
  }

  logger.debug('Detecting paper on IEEE Xplore');
