// ACM Digital Library paper detector
...
export async function detectAcm(): Promise<PaperMetadata | null> {
  if (!window.location.hostname.includes('dl.acm.org')) {
    return null;
  }

  logger.debug('Detecting paper on ACM Digital Library');
  ...
  const dateMeta = getMeta('citation_publication_date') || getMeta('citation_date');
  ...
  // Extract publisher
  const publisher = getMeta('citation_publisher');
  ...
  const abstractEl = document.querySelector('.abstractSection p') ||
                     document.querySelector('[role="paragraph"]');
